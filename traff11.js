...
function normalizeText(text) {
    ...
    function isServerCardContainer(element) {
      if (!element || element === document.body) return false;
      if (element.matches && element.matches('section.grid.items-center.gap-2, section.grid.items-center.gap-3, section.flex.items-center.w-full.justify-between.gap-1')) {
        return true;
      }
      if (element.querySelector && (element.querySelector('section.grid.items-center.gap-2') || element.querySelector('section.grid.items-center.gap-3') || element.querySelector('section.flex.items-center.w-full.justify-between.gap-1'))) {
        return true;
      }
      return false;
    }
    ...
    function findServerContainerByName(serverName) {
      ...
      const fallbackSections = Array.from(document.querySelectorAll('section.grid.items-center.gap-2, section.grid.items-center.gap-3, section.flex.items-center.w-full.justify-between.gap-1'));
    ...
    function renderTrafficStats(trafficData, config) {
      ...
      for (const cycleId in trafficData) {
        ...
        for (const serverId in cycle.server_name) {
          ...
          if (normalizedName && transfer !== undefined && max && from && to) {
